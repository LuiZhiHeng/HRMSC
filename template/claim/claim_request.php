...
<?php
    ...
    if($rs->num_rows > 0) {
        for ($i=0; $i < $rs->num_rows; $i++) { 
            ...
            <?php if($claim[10] == 3){// claim is pending ?>
                    <form class="me-1" style="display: inline-block" action="claim.php" method="POST" onsubmit="return confirms('approve the request')">
                        <input type="hidden" name="cid" value=<?= $claim[0] ?> hidden required>
                        <button type="submit" name="approve" class="btn btn-success text-light">
                            <i class="fas fa-check-circle"></i>
                        </button>
                    </form>
            <?php } 
                if($claim[10] != 5){// claim is approved / rejected ?>
                    <form class="me-1" style="display: inline-block" action="claim.php" method="POST" onsubmit="return confirms('prepare the request')">
                        <input type="hidden" name="cid" value=<?= $claim[0] ?> hidden required>
                        <button type="submit" name="prepare" class="btn btn-warning text-light">
            ...
            <?php if($claim[10] == 3){// claim is pending ?>
                    <form class="me-1" style="display: inline-block" action="claim.php" method="POST" onsubmit="return confirms('reject the request')">
                        <input type="hidden" name="cid" value=<?= $claim[0] ?> hidden required>
