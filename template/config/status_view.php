...
?>
                <td>
                    <form style="display: inline-block" action="setting.php?status=" method="POST">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 id="title" class="modal-title">Adding New Status</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="modalForm" action="setting.php?status=" method="POST" onsubmit="return confirms('Add Status')">
                    <label for="statusName">Status Name:</label>
                    <input type="hidden" id="id" name="id" hidden>
                    <input type="hidden" id="data0" class="form-control mb-2" name="statusNameOld" hidden>
                    <input type="text" id="data" class="form-control mb-2" name="statusName" required>
                    <input type="submit" id="bttn" class="btn btn-danger form-control" name="add" value="Add Status">
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    exampleModal.addEventListener('show.bs.modal', function (event) {
        ...
        if(id != "" && data != ""){
            title.innerText = "Editing Status"
            modalBodyInputId.value = id;
            modalBodyInputData0.value = data;
            modalBodyInputData.value = data;
            bttn.name = "edit";
            bttn.value = "Edit Status";
            document.getElementById("modalForm").onsubmit = function() {return confirms("Edit Status")};
        } else {
            modalBodyInputId.value = "";
            modalBodyInputData0.value = "";
            modalBodyInputData.value = "";
            bttn.name = "add";
            bttn.value = "Add Status";
            document.getElementById("modalForm").onsubmit = function() {return confirms("Add Status")};
        }
    });
</script>