...
<div class="card mb-4">
    ...
    <div class="card-body">
        <table id="datatablesSimple">
            ...
            <tbody>
<?php
    $rs = $conn->query("SELECT * FROM overtime_day_type");
    if($rs->num_rows > 0) {
...
?>
                <td>
                    <form style="display: inline-block" action="setting.php?overtime_type=" method="POST">
                    ...
                    <form style="display: inline-block" action="setting.php" method="GET">
                        ...
                        <button type="submit" class="btn btn-primary text-light" name="overtime_rule">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 id="title" class="modal-title">Adding New Day Overtime Type</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="modalForm" action="setting.php?overtime_type=" method="POST" onsubmit="return confirms('Add Overtime Day Type')">
                    <label for="overtimeTypeName">Overtime Day Type Name:</label>
                    <input type="hidden" id="id" name="id" hidden>
                    <input type="hidden" id="data0" class="form-control mb-2" name="overtimeTypeNameOld" hidden>
                    <input type="text" id="data" class="form-control mb-2" name="overtimeTypeName" required>
                    <input type="submit" id="bttn" class="btn btn-danger form-control" name="add" value="Add Overtime Type">
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    exampleModal.addEventListener('show.bs.modal', function (event) {
        ...
        if(id != "" && data != ""){
            title.innerText = "Editing Overtime Day Type"
            modalBodyInputId.value = id;
            modalBodyInputData0.value = data;
            modalBodyInputData.value = data;
            bttn.name = "edit";
            bttn.value = "Edit Overtime Day Type";
            document.getElementById("modalForm").onsubmit = function() {return confirms("Edit Overtime Day Type")};
        } else {
            modalBodyInputId.value = "";
            modalBodyInputData0.value = "";
            modalBodyInputData.value = "";
            bttn.name = "add";
            bttn.value = "Add Overtime Day Type";
            document.getElementById("modalForm").onsubmit = function() {return confirms("Add Overtime Day Type")};
        }
    });
</script>